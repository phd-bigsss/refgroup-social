...
df_pred3 %>%
  ...
  scale_colour_manual(values = c("linear" = "#1b4f72", "quadratic" = "#aeb6bf"),labels= c("Model 4","Model 5")) +
  scale_fill_manual(values = c("linear" = "#1b4f72", "quadratic" = "#aeb6bf"),labels= c("Model 4","Model 5")) +
  ...
  labs(
    ...
    caption = paste0("*Note*: N = ", nobs(int_diversity), ". 95% CIs. Based on Models in Table 1")
  ) +
  theme(
...
ggsave(plot = last_plot(), filename = "figure1.pdf", path = "output/images", width = 1.5, height = 1, scale = 15, device = "pdf", units = "cm",dpi = "retina")
...
paneldata137$n_total_gm<- MLMusingR::group_mean(paneldata137$n_total,grp = paneldata137$id)
paneldata137$n_total_gc<- MLMusingR::group_center(paneldata137$n_total,grp = paneldata137$id)

paneldata137$isei_avg_gm<- MLMusingR::group_mean(paneldata137$isei_avg,grp = paneldata137$id)
paneldata137$isei_avg_gc<- MLMusingR::group_center(paneldata137$isei_avg,grp = paneldata137$id)
...
df_models90 <- 
  bind_rows(broom::tidy(mkt_ME3, conf.int = TRUE,conf.level = 0.90)) %>% 
  filter(term %in% c("mobility_profileDownward",
                     "mobility_profileStable: high",
                     "mobility_profileStable: middle",
                     "mobility_profileStable: NEET",
...
df_models$conf.low90<- df_models90$conf.low
df_models$conf.high90<- df_models90$conf.high
...
caption = paste0("*Note*: N = ", nobs(mkt_ME3), ". 95% and 90% CIs. Multilevel hybrid model include Gender, age, network size and network status are included as controls.")

ggplot(df_models, aes(x = estimate, y = term)) + 
  geom_vline(xintercept = 0, color = "red", size = 1) + 
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.1, color = "darkgray", size = 1, position = position_dodge(width = 0.2))+
  geom_errorbarh(aes(xmin = conf.low90, xmax = conf.high90), height = 0.1, color = "darkblue", size = 1, position = position_dodge(width = 0.2))+ 
  geom_point(size = 3, position = position_dodge(width = 0.2)) + 
  labs(title = NULL, subtitle = "market-skeptical ←                                     → market-supportive", x = "Coefficients", caption = caption, y = NULL) + 
...
ggsave(plot = last_plot(), filename = "figureS02.png", path = "output/images/", width = 2, height = 1, units = "cm", scale = 14,dpi = "retina")
```
...
## RE Regression mobility profiles
...
df_models <- bind_rows(broom::tidy(mkt_just2.3, conf.int = TRUE) %>% mutate(model = "Model 1")) %>% filter(term %in% c("diversityclass","isei_c"))  
  # add_row(term = "isei_c", estimate = NA, conf.low = NA, conf.high = NA, std.error = NA, model = "Model 1", .before = 1)
df_models$term <- factor(df_models$term, levels = rev(c("isei_c","diversityclass")), labels = rev(c("**ISEI**","**Net. Diversity**")))
...
ggplot(df_models, aes(
  ...
  group = model
                ...
                ) + geom_point(size = 3, position = position_dodge(width = 0.2)) + labs(
                  title = "Socioeconomic status, network diversity and market justice preferences",
                  subtitle = "market-skeptical ← → market-supportive",
                  x = "Coefficient",
                  caption = "*Note*: Two-way fixed effects regression. Age and Age2 included",
                  y = NULL
                ) + scale_x_continuous(limits = c(-0.2, 0.1)) + theme(
...
ggsave(plot = last_plot(), filename = "fig_coefplot.png", path = "presentations/simposioUSACH2025/images", width = 2, height = 2, units = "cm", scale = 9)
```
...
df1 <- as.data.frame(paneldata137)
...
labels<- sjlabelled::get_labels(df1[,c("just_pension","just_salud","just_educ")])

df1 %>% select("just_pension","just_salud","just_educ") %>% 
  sjlabelled::set_label(label = sjlabelled::get_label(df1[,c("just_pension","just_salud","just_educ")])) %>% 
...
ggsave(plot = last_plot(), filename = "figureP1.png", path = "output/images", width = 2, height = 1.2, scale = 15, device = "png", units = "cm")
```
...
ggsave(plot = last_plot(), filename = "figureP2.png", path = "output/images", width = 1, height = 1, scale = 15, device = "png", units = "cm")
```
