...
dfreg$isei_avg_grp <- ntile(dfreg$isei_avg,3)
frq(dfreg$isei_avg_grp)
dfreg$isei_avg_grp <- ntile(dfreg$mean_isei,3)

dfreg %>% 
  group_by(isei_avg_grp) %>% 
  summarise(mean(mean_isei),min(mean_isei),max(mean_isei))
...
df1_long %>% group_by(isei_avg_grp) %>% summarise(mean(isei_avg),min(isei_avg),max(isei_avg))

table(df1_long$ola,df1_long$isei_avg_grp)

panel1<-panel_data(df1_long, id = idencuesta, wave = ola) %>% 
