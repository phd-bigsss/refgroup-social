...
dfreg$isei3res<- factor(ntile(dfreg$isei08res,4))
dfreg$isei3res_na <- factor(ifelse(is.na(dfreg$isei3res),yes = 99,no = dfreg$isei3res))
# dfreg$isei3res_na <- factor(dfreg$isei3res_na,levels = 1:4,labels = c("iseiLow","iseiMid","iseiHigh","iseiResd"))

class_t1 <- dfreg %>% filter(ola ==1) %>% select(idencuesta,ola,class1=isei3res_na)
class_t2 <- dfreg %>% filter(ola ==3) %>% select(idencuesta,ola,class3=isei3res_na)

class_w1w5<- left_join(class_t1,class_t2,by ="idencuesta")

# sjPlot::tab_xtab(class_w1w5$class1,class_w1w5$class5)
...
# t7<- df1 %>% filter(ola==7) %>%  select(id)
# df1 <- df1 %>% filter(id %in% t7$id)  
df1_atrition <- df1
...
haven::write_dta(data = as.data.frame(paneldata),path = here::here("input/data/proc/study2.dta"))
...
-0.7054360  0.0136731  0.0638493 -0.0000099  0.0908886  0.1178234 

    Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
-0.46948  0.01370  0.04613  0.00296  0.06324  0.08043 
...
mkt_just2.1 <- plm(market ~ diversityclass+isei+age,data=paneldata,weights = weights1, model="within",effect = "twoways")
mkt_just2.2 <- plm(market ~ diversityclass+isei+income+educ+age,data=paneldata,weights = weights1, model="within",effect = "twoways")
mkt_just2.3 <- plm(market ~ diversityclass+isei+isei_avg+age,data=paneldata,weights = weights1, model="within",effect = "twoways")
mkt_just2.4 <- plm(market ~ diversityclass+isei+n_total+age,data=paneldata,weights = weights1, model="within",effect = "twoways")
...
market_models_fe <- list(
  # mkt_just1_ols,
  # mkt_just2.0,
  mkt_just2,mkt_just2.1,mkt_just2.3,mkt_just2.4,mkt_just2.2,mkt_just2.5
  )

custom.coef.map = list("isei2"="Mid-low","isei3"="Mid-high",
                       "isei4"="High",
                       "isei99"="Missing",
...
screenreg(
  ...
  # file = "output/tables/market_fe.html",
...
#test tge haussman indica que es mejor usar FE con two-way 
mkt_educ <- plm(just_educ ~ diversityclass+age+isei+sex,data=paneldata,weights = weights1,model = "within",effect = "twoway")
mkt_salu <- plm(just_salud ~ diversityclass+age+isei+sex,data=paneldata,weights = weights1, model="within",effect = "twoway")
mkt_pens <- plm(just_pension ~ diversityclass+age+isei+sex,data=paneldata,weights = weights1, model="within",effect = "twoway")
services_models_fe <- list(mkt_educ,mkt_salu,mkt_pens)
screenreg(services_models_fe,stars = c(0.001,0.01,0.05,0.1))
# Between-effects  (using predictor grand mean)

# mkt_just1_w1<- lm(market ~ diversityclass+age+isei,data=paneldata,subset = time==1,weights = ponderador02)
mkt_just1_ols <- lm_robust(market ~ diversityclass+age+isei+sex+income+educ+isei_avg+n_total,data=paneldata,
                           fixed_effects = ~ id,
                          clusters = id,
                          se_type = "CR0",
                          weights = NULL)

# mkt_just1_ols <- plm(market ~ diversityclass+age+isei+sex,data=paneldata,model = "between")
# mkt_just2.0 <- plm(market~diversityclass+age+isei ,data=paneldata,weights = weights1, model="random")
...
screenreg(
  ...
  # file = "output/tables/market_fe_woutt3.html",
  custom.note = "%stars; Standard errors in parentheses. Standardized coefficients. Models include age as control."
)
```
