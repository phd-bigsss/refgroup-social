...
pacman::p_load(dplyr,readxl,sjmisc, sjlabelled, questionr, car,here,ggplot2,DIGCLASS)# load libraries
...
```{r social-class}
# elsoc_long_2016_2022.2$isco08 = as.numeric(isco88_to_isco08(elsoc_long_2016_2022.2$isco88))
elsoc_long_2016_2022.2 <- 
elsoc_long_2016_2022.2 %>%
  mutate(
...
elsoc_long_2016_2022.2$isco88_2 = as.numeric(isco08_to_isco88(elsoc_long_2016_2022.2$isco08))
summary(elsoc_long_2016_2022.2$isco88_2)
summary(elsoc_long_2016_2022.2$isco88)

elsoc_long_2016_2022.2$isco88_3<- ifelse(is.na(elsoc_long_2016_2022.2$isco88_2),yes = elsoc_long_2016_2022.2$isco88,elsoc_long_2016_2022.2$isco88_2)
summary(elsoc_long_2016_2022.2$isco88_3)

# elsoc_long_2016_2022.2 %>% select(idencuesta,ola,isco88res,isco08res,isco88_2,isco88_3,is_supervisor_r,self_employed_r,n_employees_r) %>% arrange(idencuesta) %>% View()
...
elsoc_long_2016_2022.2$isco88res_2 = as.numeric(isco08_to_isco88(elsoc_long_2016_2022.2$isco08res))
summary(elsoc_long_2016_2022.2$isco88res_2)
summary(elsoc_long_2016_2022.2$isco88res)

elsoc_long_2016_2022.2$isco88res_3<- ifelse(is.na(elsoc_long_2016_2022.2$isco88res_2),yes = elsoc_long_2016_2022.2$isco88res,elsoc_long_2016_2022.2$isco88res_2)
summary(elsoc_long_2016_2022.2$isco88res_3)
# elsoc_long_2016_2022.2 %>% select(idencuesta,ola,isco88res,isco08res,isco88res_2,is_supervisor_r,self_employed_r,n_employees_r) %>% arrange(idencuesta) %>% View()
...
# elsoc_long_2016_2022.2 %>% select(idencuesta,ola,isco88res,is_supervisor_r,self_employed_r,n_employees_r) %>% arrange(idencuesta)
...
elsoc_long_2016_2022.2$egp8_r_label <- car::recode(var = elsoc_long_2016_2022.2$egp11_r_label,
...
elsoc_long_2016_2022.2$egp8_r_label_na <- elsoc_long_2016_2022.2$egp8_r_label
egp8lab <- levels(elsoc_long_2016_2022.2$egp8_r_label_na)
elsoc_long_2016_2022.2$egp8_r_label_na <- ifelse(elsoc_long_2016_2022.2$m02 %in% c(4,5,6,7,8,9),yes = 9,no= elsoc_long_2016_2022.2$egp8_r_label_na)
elsoc_long_2016_2022.2$egp8_r_label_na <- factor(elsoc_long_2016_2022.2$egp8_r_label_na,labels = c(egp8lab,"Never worked and long-term unemployed"))
frq(elsoc_long_2016_2022.2$egp8_r_label_na)
```


```{r}
# elsoc_long_2016_2022.2 %>% select(idencuesta,ola,isco88res,isco08res,isco88res_2,isco88res_3,is_supervisor_r,self_employed_r,n_employees_r,egp11_r,egp7_r,egp5_r,egp3_r) %>% arrange(idencuesta) %>% View()
...
elsoc_long_2016_2022.2 %>%
  group_by(ola) %>% 
  summarise(perc=mean(porc_high,na.rm = T),Status="1. high-status"),
elsoc_long_2016_2022.2 %>%
  group_by(ola) %>% 
  summarise(perc=mean(porc_middle,na.rm = T),Status="2. middle-status"),
elsoc_long_2016_2022.2 %>%
  group_by(ola) %>% 
  summarise(perc=mean(porc_low,na.rm = T),Status="3. low-status")
) %>% 
  na.omit() %>% 
...
#Filter 2
df_filter2<- data.frame(frq(dfreg$idencuesta)) %>% select(idencuesta=val,frq)
df_filter2$keep <- df_filter$frq>=3
table(df_filter2$keep)

dfreg$frq <- NULL
dfreg$keep <- NULL
dfreg <-  dfreg %>% left_join(df_filter2,by = "idencuesta")
dfreg <-  dfreg %>%  filter(keep=="TRUE")
table(dfreg$keep)


library(estimatr);library(lme4)
education <- 
list(
  lmer(just_educ~porc_low + porc_high + (1|idencuesta),data = dfreg),
  lmer(just_educ~isei_avg + ola + educyear + quintil1+ (1|idencuesta),data = dfreg),
  lmer(just_educ~isei_avg + diversity + ola + educyear + quintil1+ (1|idencuesta),data = dfreg),
  lmer(just_educ~isei_avg + diversityclass +  ola + educyear + quintil1+ (1|idencuesta),data = dfreg)
)

texreg::screenreg(education, include.ci = FALSE)

health <- 
list(
  lmer(just_salud~porc_low + porc_high+ (1|idencuesta),data = dfreg),
  lmer(just_salud~isei_avg + ola + educyear + quintil1+ (1|idencuesta),data = dfreg),
  lmer(just_salud~isei_avg + diversity + ola + educyear + quintil1+ (1|idencuesta),data = dfreg),
  lmer(just_salud~isei_avg + diversityclass +  ola + educyear + quintil1+ (1|idencuesta),data = dfreg)
  )

texreg::screenreg(health,include.ci = FALSE)

pensions <- 
list(
  lmer(just_pension~porc_low + porc_high+ (1|idencuesta),data = dfreg),
  lmer(just_pension~isei_avg + ola + educyear + quintil1+ (1|idencuesta),data = dfreg),
  lmer(just_pension~isei_avg + diversity + ola + educyear + quintil1+ (1|idencuesta),data = dfreg),
  lmer(just_pension~isei_avg + diversityclass + ola + educyear + quintil1+ (1|idencuesta),data = dfreg)
)

texreg::screenreg(pensions,include.ci = FALSE)

market <- 
list(
  lmer(market~ ola + diversity + (1|idencuesta),data = dfreg),
  lmer(market~ diversity + ola + quintil1+ (1|idencuesta),data = dfreg),
  lmer(market~ diversity + ola + educyear + quintil1 + (1|idencuesta),data = dfreg),
  lmer(market~ diversity + ola + educyear + quintil1 + egp8_r_label_na + (1+diversity|idencuesta),data = dfreg)
)

texreg::screenreg(market,include.ci = FALSE)

market2 <- 
list(
  lmer(market2~ ola + diversity + (1|idencuesta),data = dfreg),
  lmer(market2~ diversity + ola + quintil1+ (1|idencuesta),data = dfreg),
  lmer(market2~ diversity + ola + educyear + quintil1 + (1|idencuesta),data = dfreg),
  lmer(market2~ diversity + ola + educyear + quintil1 + egp8_r_label_na + (1|idencuesta),data = dfreg)
)

texreg::screenreg(market2,include.ci = FALSE)

salarygaps <- 
list(
  lmer(justgapptile~ ola + diversity + (1|idencuesta),data = dfreg),
  lmer(justgapptile~ diversity + ola + quintil1+ (1|idencuesta),data = dfreg),
  lmer(justgapptile~ diversity + ola + educyear + quintil1 + (1|idencuesta),data = dfreg),
  lmer(justgapptile~ diversity + ola + educyear + quintil1 + egp8_r_label_na + (1|idencuesta),data = dfreg),
  lmer(justgapptile~ diversity + ola + educyear + quintil1 + egp8_r_label_na +  percgapptile + (1|idencuesta),data = dfreg)
  )
...
elsoc_long_2016_2022.2 %>%
  filter(ola==1) %>% 
  group_by(egp8_r_label) %>% 
  summarise(perc=mean(just_educ,na.rm = T),Status="1. Education"),
elsoc_long_2016_2022.2 %>%
  filter(ola==1) %>%   
  group_by(egp8_r_label) %>% 
  summarise(perc=mean(just_salud,na.rm = T),Status="2. Health"),
elsoc_long_2016_2022.2 %>%
  filter(ola==1) %>%   
  group_by(egp8_r_label) %>% 
  summarise(perc=mean(just_pension,na.rm = T),Status="3. Pensions")
) %>% 
  na.omit() %>% 
  ggplot(aes(x=factor(egp8_r_label),y = perc, group=Status, color=Status)) +
  geom_point()+
...
elsoc_long_2016_2022.2 %>%
  ...
  na.omit() %>% 
  ggplot(aes(x=factor(quintil1),y = perc, group=Status, color=Status)) +
  geom_point()+
  geom_line() +
  scale_y_continuous(limits = c(1,5)) +
  # scale_x_discrete(labels=c("2016","2018","2021")) +
  labs(title = "Market-based distribution of social services by income (Wave 1)",x="Income",y=NULL) +
  theme()
...
elsoc_long_2016_2022.2 %>%
  ...
  na.omit() %>% 
  ggplot(aes(x=factor(educ),y = perc, group=Status, color=Status)) +
  geom_point()+
  geom_line() +
  scale_y_continuous(limits = c(1,5)) +
  # scale_x_discrete(labels=c("2016","2018","2021")) +
  labs(title = "Market-based distribution of social services by education (Wave 1)",x="Income",y=NULL) +
  theme()
```
...
# Example data
data <- data.frame(
  subject = c("A", "A", "A", "A", "B", "B", "B", "B"),
  time = c(1, 2, 3, 4, 1, 2, 3, 4),
  Y = c(1, 2, NA, 4, 3, 4, NA, 6)
)

# Copy values from time 4 to time 3 within each subject
data_imputed <- data %>%
  group_by(subject) %>%
  mutate(Y = if_else(time == 3 & is.na(Y), last(Y[time == 4]), Y)) %>%
  ungroup()
